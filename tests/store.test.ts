...
test('v0.1.0 表结构原位升级保留频道名、密钥和历史', async () => {
  ...
  try {
    ...
    db.exec('DROP INDEX sessions_user; ALTER TABLE sessions DROP COLUMN user_id; DROP TABLE users; DROP INDEX feeds_owner; DROP INDEX credentials_owner; DROP INDEX import_jobs_owner; ALTER TABLE feeds DROP COLUMN owner_id; ALTER TABLE credentials DROP COLUMN owner_id; ALTER TABLE import_jobs DROP COLUMN owner_id;');
