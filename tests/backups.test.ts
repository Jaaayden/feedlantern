...
test('完整备份跨密钥恢复管理员、凭据、频道名称和历史，失败事务不改原数据', () => {
  ...
  try {
    ...
    const snapshot = snapshotSchema.parse({ format: 'feedlantern-backup', version: 2, appVersion: '0.2.0', createdAt: new Date().toISOString(), security: { allowedHosts: [], dnsOverHttps: false }, tables: a.exportTables() });
...
test('日期来源随备份恢复，旧备份无来源时按绝对时间读取', () => {
  ...
  try {
    ...
    const snapshot = { format: 'feedlantern-backup', version: 2, appVersion: '0.2.2', createdAt: new Date().toISOString(), security: { allowedHosts: [], dnsOverHttps: false }, tables: store.exportTables() };
