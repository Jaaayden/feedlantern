...
export default function App() {
  ...
  const acceptAuth = useCallback((state: AuthState) => { authGeneration.current++; setFeeds([]); setCsrfToken(state.csrfToken); setAuth(state); setPage('feeds'); setQuery(''); setError(''); }, []);
  const forgetAuth = useCallback(() => { authGeneration.current++; setCsrfToken(undefined); setAuth(previous => previous ? { ...previous, authenticated: false, csrfToken: undefined } : null); setFeeds([]); setEditor(undefined); setDetailId(null); setSettingsId(null); setEditorReturnId(null); }, []);
