...
function AuthScreen({ state, authenticated }: { state: AuthState; authenticated: (state: AuthState) => void }) {
  ...
  </form><p className="auth-note"><KeyRound size={14} />{state.setupRequired ? '只有持有本机设置码的人可以创建管理员。' : '忘记密码时，可在服务主机运行 pnpm admin:reset。'}</p><span className="version">{state.version} · <a href="https://github.com/Jaaayden/feedlantern" target="_blank" rel="noreferrer">GitHub · Jaaayden/feedlantern</a></span></div></section></div>;
}
...
function ChannelTitleSetting({ feed, saved }: { feed: Feed; saved: () => void }) {
  const [title, setTitle] = useState(feed.channelTitle ?? feed.name);
  const [error, setError] = useState('');
  const [busy, setBusy] = useState(false);
  ...
  }}><Field label="阅读器中的订阅名称" hint="修改 RSS 频道标题，订阅地址保持不变。已有阅读器可能保留原名称。"><input required maxLength={200} value={title} onChange={e => setTitle(e.target.value)} /></Field><ErrorNote error={error} /><button className="button small" disabled={busy}>保存订阅名称</button></form></details>;
}
