...
function AuthScreen({ state, authenticated }: { state: AuthState; authenticated: (state: AuthState) => void }) {
  ...
  </form><p className="auth-note"><KeyRound size={14} />{state.setupRequired ? '只有持有本机设置码的人可以创建管理员。' : '忘记密码时，可在服务主机运行 pnpm admin:reset。'}</p><span className="version">{state.version}</span></div></section></div>;
}
...
function Editor({ initial, saved, cancel }: { initial?: Feed; saved: (id: string) => void; cancel: () => void }) {
  ...
  useEffect(() => { void api.credentials.list().then(setCredentials).catch(error => setError(messageOf(error))); return () => { if (sessionRef.current) void api.browser.close(sessionRef.current).catch(() => {}); }; }, []);
  ...
  async function save(event: FormEvent) {
    ...
    try { const body: FeedInput = { name, url, credentialId: credentialId || null, rules, ruleOrigins: origins, intervalMinutes, waitMs, waitForSelector: waitForSelector || undefined }; const result = initial ? await api.feeds.update(initial.id, body) : await api.feeds.create(body); saved(result.feed.id); }
    catch (error) { setError(messageOf(error)); } finally { setBusy(''); }
  }
...
export default function App() {
  ...
  return <div className="app-layout"><aside className="sidebar"><Brand /><div className="workspace-label">个人工作台</div><nav aria-label="主导航"><button className={page === 'feeds' ? 'selected' : ''} onClick={() => navigate('feeds')}><Rss size={19} />订阅管理<span>{feeds.length}</span></button><button className={page === 'credentials' ? 'selected' : ''} onClick={() => navigate('credentials')}><Cookie size={19} />Cookie 凭据</button><button className={page === 'settings' ? 'selected' : ''} onClick={() => navigate('settings')}><Settings2 size={19} />设置</button></nav><div className="sidebar-bottom"><div className="local-label"><span /> 自托管 · 独立运行</div><div className="account"><span className="avatar">{auth.username?.slice(0, 1).toUpperCase()}</span><div><strong>{auth.username}</strong><small>管理员</small></div><button className="icon-button" aria-label="退出登录" onClick={() => void logout()}><LogOut size={17} /></button></div><span className="version">{auth.version}</span></div></aside><div className="main-shell"><header className="topbar"><span><span className="muted">工作台</span><span className="slash">/</span>{page === 'feeds' ? editor !== undefined ? '创建与编辑' : '订阅管理' : page === 'credentials' ? 'Cookie 凭据' : '设置'}</span><span className="topbar-status"><ShieldCheck size={14} /> 已安全登录 <button className="icon-button mobile-logout" aria-label="退出登录" onClick={() => void logout()}><LogOut size={15} /></button></span></header><main className="main-content">
    {page === 'credentials' ? <CredentialsView notify={notify} /> : page === 'settings' ? <SettingsView auth={auth} logout={forgetAuth} notify={notify} /> : editor !== undefined ? <Editor key={editor?.id ?? 'new'} initial={editor ?? undefined} cancel={() => setEditor(undefined)} saved={id => { setEditor(undefined); setDetailId(id); notify('订阅已保存'); void loadFeeds(); }} /> : <><div className="page-heading"><div><span className="section-kicker">LESS CHECKING. MORE READING.</span><h1>你的订阅，持续点亮。</h1><p>把关注的网页汇集在这里，新内容会自动抵达。</p></div><button className="button primary" onClick={() => setEditor(null)}><Plus size={17} />新建订阅</button></div><div className="stats-row"><div><span>全部订阅</span><strong>{feeds.length}<small>个来源</small></strong></div><div><span>正在关注</span><strong>{feeds.filter(feed => feed.enabled).length}<small>自动更新</small></strong></div><div><span>已收集内容</span><strong>{feeds.reduce((sum, feed) => sum + feed.itemCount, 0)}<small>条记录</small></strong></div></div><div className="list-toolbar"><h2>订阅列表 <span>{feeds.length}</span></h2><div className="input-icon search-input"><Search size={16} /><input aria-label="搜索订阅" placeholder="搜索名称或网址" value={query} onChange={e => setQuery(e.target.value)} /></div></div><ErrorNote error={error} />{loading && !feeds.length ? <Busy label="正在加载订阅" /> : filtered.length ? <div className="feed-grid">{filtered.map(feed => <article className="feed-card" key={feed.id}><div className="card-top"><span className="soft-icon"><Globe2 size={22} /></span><span className={`badge ${feed.lastError ? 'bad' : feed.enabled ? 'good' : ''}`}>{feed.lastError ? '需要检查' : feed.enabled ? '更新中' : '已暂停'}</span></div><button className="card-title" aria-label={`查看订阅 ${feed.name}`} onClick={() => setDetailId(feed.id)}>{feed.name}</button><p className="feed-host">{hostLabel(feed.url)}</p>{feed.lastError && <p className="card-error">{feed.lastError}</p>}<div className="card-footer"><span>{feed.itemCount} 条内容 · 每 {feed.intervalMinutes} 分钟</span><button className="icon-button" aria-label={`编辑订阅 ${feed.name}`} onClick={() => setEditor(feed)}><Pencil size={16} /></button></div><small className="last-updated">最近成功：{dateLabel(feed.lastSuccessAt)}</small></article>)}</div> : <div className="empty-state"><span className="large-icon"><Rss size={32} /></span><h2>{query ? '没有找到匹配的订阅' : '从一个值得关注的网址开始'}</h2><p>{query ? '尝试其他名称或网址。' : '输入网页地址，自动识别内容，预览后即可生成 RSS。'}</p>{!query && <button className="button" onClick={() => setEditor(null)}><Plus size={16} />创建第一条订阅</button>}</div>}<div className="dashboard-note"><Sparkles size={17} /><span>自动识别先行，手动调整随时可用。</span><span className="muted">你的内容，你来选择。</span></div></>}
  </main><footer className="page-footer"><span>FeedLantern · 订阅灯</span><span>让值得关注的内容，自己找到你。</span></footer></div>{detailId && <FeedDetail id={detailId} close={closeDetail} edit={feed => { setDetailId(null); setEditor(feed); }} changed={() => void loadFeeds()} notify={notify} />}{toast && <div className="toast" role="status"><Check size={16} />{toast}</div>}</div>;
}
