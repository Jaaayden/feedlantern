...
export function BackupPanel({ setup = false, onRestored, onImported }: { setup?: boolean; onRestored: () => void; onImported?: () => void }) {
  ...
  return <section className="panel settings-form"><h2>{setup ? '从备份恢复' : '备份与迁移'}</h2><p>完整备份包含 Cookie 和订阅密钥，使用独立密码加密。配置导出包含应用及服务器设置和 Bark 密钥，请私密保管；不包含网站 Cookie 和 RSS 密钥。</p><div className="detail-actions">{(setup ? [['restore', '恢复完整备份']] : [['export', '完整备份'], ['restore', '完整恢复'], ['config', '配置导出'], ['import', '配置导入']]).map(([key, label]) => <button type="button" className="button small" disabled={busy} key={key} onClick={() => { reset(); setMode(key as typeof mode); }}>{label}</button>)}</div>
    {mode && <form onSubmit={e => { e.preventDefault(); void run(); }}>
      ...
      {preview && <><h3>导入预览</h3>{preview.settings ? <details><summary>将覆盖的应用及服务器设置（Bark 密钥已隐藏）</summary><pre>{JSON.stringify(preview.settings, null, 2)}</pre></details> : <p>文件没有应用设置，配置导入保留现有设置。</p>}{mode === 'restore' ? <><p>版本 {String(preview.appVersion)} · 时间 {String(preview.createdAt)}</p><p>管理员 {String(preview.username)} · {String(preview.feeds)} 个订阅 · {String(preview.credentials)} 组凭据 · {String(preview.items)} 条历史</p><p>恢复会替换现有数据并退出登录，之后使用备份中的管理员账号登录。新版备份会恢复应用及服务器设置，监听地址与端口重启生效；旧版备份保留当前服务器设置。</p><details><summary>安全配置差异</summary><pre>{JSON.stringify({ '备份配置': preview.sourceSecurity, '导入前目标配置': preview.targetSecurity }, null, 2)}</pre></details></> : <>{(preview.entries as Array<{ name: string; duplicate: boolean; needsCookie: boolean }>).map((e, i) => <p key={i}>{e.name} · {e.duplicate ? '跳过重复' : e.needsCookie ? '导入并暂停，等待绑定 Cookie' : '新增订阅'}</p>)}</>}
