...
升级会事务迁移用户及资源归属，保留原管理员密码和所有 RSS 地址，并要求原管理员重新登录。请先保存升级前的完整数据卷；回退程序时同时回退数据卷。管理员登录后从“用户管理”创建、停用或重置普通用户，`pnpm admin:reset` 仅影响管理员的密码及登录会话。详见[备份与迁移](backup-and-migration.md)。
